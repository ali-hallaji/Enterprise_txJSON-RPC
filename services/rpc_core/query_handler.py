...
def get_server(server):
    server = Server('http://aliehsanmilad:Key1_s!3cr3t@{0}'.format(server))
    return server
...
def send_request(*args):
    return select_send_request(settings.CORE_SERVICES, *args)


def main_send_request(*args):
    return select_send_request(settings.MAIN_SERVICES, *args)
